...
function test_calendar_styles() {  
	...
	wp_register_style( 'test_calendar_fullcalendar_style', plugins_url( 'lib/monthly/css/monthly.css', __FILE__ ));
	wp_enqueue_style( 'test_calendar_fullcalendar_style' ); 
}
...
function test_calendar_scripts() {  
	...
	wp_register_script( 'test_calendar_fullcalendar_script', plugins_url( 'lib/monthly/js/monthly.js', __FILE__ ), array( 'jquery' )); 
	wp_enqueue_script( 'test_calendar_fullcalendar_script' );  
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
			...
			$item = [
				'id' => get_the_id(),
				'name' => get_the_title(),
				'startdate' => get_field('event_date', get_the_id()),
				'color' => '#cd2653',
		...
		file_put_contents(plugin_dir_path( __FILE__ ).'events.json', $json);
